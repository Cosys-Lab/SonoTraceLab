                                             ...
                                             int numTriangles, int numRays)
{
    ...
    if (intersectMoller(v0, v1, v2, edge1, edge2, start, dir)) {
        ...
        atomicAdd(counter, 1 );
        //results[triangleIdx + rayIdx*numTriangles] = 1;
    }
}
                                                   ...
                                                   int numTriangles, int numRays)
{
    ...
    if(triangleIdx < numTriangles && rayIdx < numRays){
        checkRayTriangleIntersection(vertices, raysFrom, directions, results, counter, triangleIdx, rayIdx, numTriangles, numRays);
    }
}
...
void decompressResults(int* results, int triangleCount, int numTriangles, int numRays, int* activeRows, int* activeColumns)
{
    int activeCount = 0;  // To keep track of the number of active entries
    ...
    for (int triangleIdx = 0; triangleIdx < triangleCount; ++triangleIdx)
    {
        for (int rayIdx = 0; rayIdx < numRays; ++rayIdx)
        {
            ...
            // Extract the bit corresponding to the current (triangleIdx, rayIdx) pair
            int bit = (results[tempRayIdx * numTriangles + triangleIdx] >> bitOffset) & 1;
            if (bit == 1)
            {
                // printf("found the same hit: triangleIdx: %i rayIdx: %i\n", triangleIdx, rayIdx);
                activeRows[activeCount] = triangleIdx;
                activeColumns[activeCount] = rayIdx;
                activeCount++;
            }
        }
    }
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )
{
    ...
    cudaMallocManaged(&d_triangleCount, sizeof(int)); 
    int initialValue = 0;
    cudaMemcpy(d_triangleCount, &initialValue, sizeof(int), cudaMemcpyHostToDevice);
    ...
    checkRayTriangleIntersectionKernel<<<numBlocks, threadsPerBlock>>>(d_vertices, d_raysFrom, d_directions, d_results, d_triangleCount, numTriangles, numRays);

    /* Wait for the kernel to finish. */
    cudaDeviceSynchronize();

    /* Return the data from GPU device memory to host memory. */
    int triangleCount;
    int* results = new int[numTriangles * numRaysLogicalStorage];
    cudaMemcpy(&triangleCount, d_triangleCount, sizeof(int), cudaMemcpyDeviceToHost);
    cudaMemcpy(results, d_results, sizeof(int)*numRaysLogicalStorage*numTriangles, cudaMemcpyDeviceToHost);
    // printf("Intersection count: %i\n", triangleCount);
    ...
    */
    ...
    /* Allocate arrays to store active rows and columns. */
    mxArray *mxActiveRows = mxCreateNumericMatrix(triangleCount, 1, mxINT32_CLASS, mxREAL);
    mxArray *mxActiveColumns = mxCreateNumericMatrix(triangleCount, 1, mxINT32_CLASS, mxREAL);
    int *activeRows = (int *)mxGetData(mxActiveRows);
    int *activeColumns = (int *)mxGetData(mxActiveColumns); 

    /* Decrompress the results to active rows and columns. */
    printf("0");
    decompressResults(results, triangleCount, numTriangles, numRays, activeRows, activeColumns);
    printf("1");
    /* Wrap the results up as MATLAB arrays for return. */
    plhs[0] = mxActiveRows;
    plhs[1] = mxActiveColumns;
    printf("2");
